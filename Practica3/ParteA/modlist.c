...
static ssize_t myread(struct file *filp, char __user *buf, size_t len, loff_t *off) {
	...
	list_for_each(cur_node, &mylist){
		if(contadorSeguridad > len){
			printk(KERN_INFO "Cuidado, no hay tanto espacio para tanta lectura!\n");
			//return 0;
			break;
		}
		...
		dest += sprintf(dest, "%i\n", item->data);
		printk(KERN_INFO "valor numerico dest: %d\n", sprintf(dest, "%i\n", item->data));
...
static ssize_t mywrite(struct file *filp, const char __user *buf, size_t len, loff_t *off) {
	...
	printk(KERN_INFO "VALOR DE len EN WRITE: %d\n", len);
